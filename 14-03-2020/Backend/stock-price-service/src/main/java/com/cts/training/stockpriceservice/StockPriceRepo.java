...
@Query("SELECT new com.cts.training.stockpriceservice.StockPriceOnPeriod(companyCode,stockExchange,date,AVG(currentPrice)) " + " FROM StockPrice "+ " WHERE companyCode=?1 AND stockExchange=?2 AND date BETWEEN ?3 AND ?4 "+ "GROUP BY date")
